  ...
  def forward_event
    if !params[:event_id].blank? && !params[:user_ids].blank?
      ids_array = params[:user_ids].split(',').map {|s| s.to_i } # convert into array
       ...
       if @check.blank?  
       @recipient = User.find(id)
       if @notification = Notification.create!(recipient: @recipient, actor: request_user, action: User.get_full_name(request_user) + " has forwarded you and event.", notifiable: @event, url: "/admin/events/#{@event.id}", notification_type: 'mobile', action_type: "forward_event")
    ...
    if !params[:event_shared].blank? && params[:event_shared] ==  'true' 
      if !params[:sender_token].blank? && !params[:event_id].blank?
        ...
        @pubnub = Pubnub.new(
          ...
          if @notification = Notification.create!(recipient: @recipient, actor: @sender, action: User.get_full_name(@sender) + " shared an event with you.", notifiable: @event, url: "/admin/events/#{@event.id}", notification_type: 'mobile', action_type: "share_event")
