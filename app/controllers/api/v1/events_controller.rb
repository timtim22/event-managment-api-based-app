class Api::V1::EventsController < Api::V1::ApiMasterController
          ...
          @events = []
           ...
           if !common_ids.blank?
            ...
            location_based_ids = @events.map {|event| event.id }
             ...
             pass_based_events = filter_events_by_pass
             pass_based_ids = pass_based_events.map {|event| event.id }
             common_ids = price_based_ids & category_based_ids & pass_based_ids & location_based_ids
             if !common_ids.blank?
              @events = Event.where(id: common_ids).page(params[:page]).per(get_per_page).map {|event| get_simple_event_object(event) }
             else
              @events = []
  ...
  def filter_events_by_price(price)
      ...
      if price == 0
        ...
        @events = Event.where("price != ?", "0.00").where("price < ?", threshhold).or(Event.where("end_price != ?", "0.00").where("end_price < ?", threshhold).or(Event.where("start_price < ?", threshhold))).sort_by_date.page(params[:page]).per(get_per_page)
      elsif price > 60
        @events = Event.where("price != ?", "0.00").where("price > ?", threshhold).or(Event.where("end_price != ?", "0.00").where("end_price > ?", threshhold).or(Event.where("start_price > ?", threshhold))).sort_by_date.page(params[:page]).per(get_per_page)
      else 
       
        @events = Event.where("price != ?", "0.00").where("price < ?", price).or(Event.where("end_price != ?", "0.00").where("end_price < ?", price).or(Event.where("start_price < ?", price))).sort_by_date.page(params[:page]).per(get_per_page)
