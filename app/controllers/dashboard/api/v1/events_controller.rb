 class Dashboard::Api::V1::EventsController < Dashboard::Api::V1::ApiMasterController
  ...
  if @error_messages.blank?
    ...
    @event.start_time =  DateTime.parse(params[:start_time]).strftime("%H:%M:%S")
    @event.end_time = DateTime.parse(params[:end_time]).strftime("%H:%M:%S")
    ...
    if @event.save
      ...
      params[:event_dates].map { |date| @event.child_events.create!(
            ...
            start_time: DateTime.parse(params[:start_time]).strftime("%H:%M:%S"),
            end_time: DateTime.parse(params[:end_time]).strftime("%H:%M:%S"),
