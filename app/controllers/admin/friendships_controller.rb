class Admin::FriendshipsController < Admin::AdminMasterController
  ...
  def send_request
    ...
    if @friend_request.save
     ...
     if @notification = Notification.create(recipient: @friend, actor: @sender, action: get_full_name(@sender) + " sent you a friend request", notifiable: @friend_request, resource: @friend_request, url: '/admin/friend-requests', notification_type: 'web')  
      @pubnub = Pubnub.new(
...
def accept_request
  ...
  if request.save
    ...
    if new_request.save
     ...
     if @notification = Notification.create(recipient: request.user, actor: current_user, action: get_full_name(current_user) + " accepted your friend request", notifiable: request, resource: request, url: '/admin/my-friends', notification_type: 'web')
