  ...
  def create
    @event = current_user.events.new
     if params[:free_ticket].blank? && params[:paid_ticket].blank? && params[:pay_at_door].blank? && params[:pass].blank?
      flash[:alert_danger] = "One of the adminssion process must be defined."
      render :new
      return
     end
    ...
    @pubnub = Pubnub.new(
      ...
      if(params[:start_date] != params[:end_date])
         ...
         if !params[:pass].blank?
            ...
            passes.each do |pass|
             if @pass = @event.passes.create!(user: current_user, title: pass["title"], quantity: pass["quantity"], valid_from: pass["valid_from"].to_date, valid_to: pass["valid_to"].to_date, validity: pass["valid_to"], ambassador_rate: pass["ambassador_rate"], description: pass["description"], redeem_code: generate_code)
